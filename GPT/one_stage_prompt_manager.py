...
class OneStagePromptManager(object):
    """
    Manages prompt generation for one-stage vision-and-language navigation.
    
    This class constructs structured prompts that integrate:
    - Navigation instructions and history
    - Spatial graph representations  
    - Visual observations from multiple viewpoints
    - Action planning and trajectory tracking
    - Extended destination information for enhanced spatial reasoning
    
    The prompt manager supports both standard text-based and JSON-formatted outputs
    for different LLM interaction modes.
    """
    def __init__(self, args):
        """
        Initialize the OneStagePromptManager with batch processing capabilities.
        
        Args:
            args: Configuration arguments containing batch_size and other hyperparameters
        """
        self.args = args
        
        # Initialize batch-wise data structures for multi-environment processing
        self.history  = ['' for _ in range(self.args.batch_size)]  # Navigation history for each environment
        self.nodes_list = [[] for _ in range(self.args.batch_size)]  # List of explored nodes/viewpoints
        self.node_imgs = [[] for _ in range(self.args.batch_size)]  # Images associated with each node
        self.graph  = [{} for _ in range(self.args.batch_size)]  # Spatial connectivity graph
        self.trajectory = [[] for _ in range(self.args.batch_size)]  # Agent's movement trajectory
        self.planning = [["Navigation has just started, with no planning yet."] for _ in range(self.args.batch_size)]  # Long-term planning

    def get_action_concept(self, rel_heading, rel_elevation):
        """
        Convert relative heading and elevation angles to human-readable action descriptions.
        
        This method translates 3D spatial relationships into intuitive movement commands
        that can be easily understood by language models.
        
        Args:
            rel_heading (float): Relative heading angle in radians [-π, π]
            rel_elevation (float): Relative elevation angle in radians
            
        Returns:
            str: Human-readable action description ('go up', 'turn left', etc.)
        """
        # Handle vertical movement first (elevation takes priority)
        if rel_elevation > 0:
            action_text = 'go up'
        elif rel_elevation < 0:
            action_text = 'go down'
        else:
            # Handle horizontal movement based on heading angle
            if rel_heading < 0:  # Left side movements
                if rel_heading >= -math.pi / 2:  # Small left turn
                    action_text = 'turn left'
                elif rel_heading < -math.pi / 2 and rel_heading > -math.pi * 3 / 2:  # Large left turn
                    action_text = 'turn around'
                else:  # Wrap-around to right
                    action_text = 'turn right'
            elif rel_heading > 0:  # Right side movements
                if rel_heading <= math.pi / 2:  # Small right turn
                    action_text = 'turn right'
                elif rel_heading > math.pi / 2 and rel_heading < math.pi * 3 / 2:  # Large right turn
                    action_text = 'turn around'
                else:  # Wrap-around to left
                    action_text = 'turn left'
            elif rel_heading == 0:  # Straight ahead
    ...
    def make_r2r_prompts(self, obs, cand_inputs, t):
        extended_instruction = getattr(self.args, 'extended_instruction', False)
        ext_instruction_file = getattr(self.args, 'extended_instr_file', None)
        ...
        + """ At each step, a series of images corresponding to the places you have explored and have observed will be provided to you. Target detection boxes in images may highlight objects relevant to the optimal navigation path, so take them into account as needed."""
        ...
        map_info = """'Map' refers to the connectivity between the places you have explored and other places you have observed. This map is dynamically updated; some previously seen places/connections might be optimized (pruned) for clarity."""
        ...
        destination_info = """'Possible destination info' describes a possible spatial info of objects for the target destination. This is intended as a reference and may not be completely accurate."""
        ...
        # **Modification point 1: Update thought requirements in task_description**
        sys_modify = True
        if sys_modify: 
            thought1 = """Your answer should be JSON format and must include three fields: 'Thought', 'New Planning', and 'Action'. You need to combine 'Instruction', 'Trajectory', 'Map', 'Supplementary Info', your past 'History', 'Previous Planning', 'Possible Destination Info', and 'Action Options' the provided images to think about what to do next and why, and complete your thinking into 'Thought'."""
            thought = thought1
        ...
        for i in range(batch_size):
            ...
            # print(f"{navigation_alerts_text}")
            # input()
            ext_instruction = ""
            if extended_instruction:
                with open(ext_instruction_file, 'r') as f:
                    ext_data = json.load(f)
                for item in ext_data:
                    if item['scan'] == obs[i]['scan'] and item['path_id'] == obs[i]['path_id'] and item['instruction'] == instruction:
                        ext_instruction = item['final_destination_spatial_relations']
                        break

            if t == 0:
                if ext_instruction is not None and extended_instruction:
                    prompt = f"""Instruction: {instruction}\nHistory: {init_history}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nAction options (step {str(t)}): {action_options_batch[i]}\nPossible destination info: {ext_instruction}\n"""
                else:
                    prompt = f"""Instruction: {instruction}\nHistory: {init_history}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nAction options (step {str(t)}): {action_options_batch[i]}"""
            else:
                if ext_instruction is not None and extended_instruction:
    ...
    def make_r2r_json_prompts(self, obs, cand_inputs, t):
        ...
        option = """'Action options' are some actions that you can take at this step."""
        pre_planning = """'Previous Planning' records previous long-term multi-step planning info that you can refer to now."""
        destination_info = """'Possible Destination Info' describes a possible spatial layout of objects for the target destination. This is intended as a reference and may not be completely accurate."""
        ...
        for i in range(batch_size):
            ...
            if t == 0:
                if ext_instruction is not None and extended_instruction:
                    prompt = f"""Instruction: {instruction}\nHistory: {init_history}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nPossible Destination Info (For reference only, may not be completely accurate): {ext_instruction}\nAction Options (step {str(t)}): {action_options_batch[i]}\n"""
                else:
                    prompt = f"""Instruction: {instruction}\nHistory: {init_history}\nTrajectory: {trajectory_text}\nMap:{graph_text}\nSupplementary Info: {graph_supp_text}\nPrevious Planning:\n{self.planning[i][-1]}\nAction Options (step {str(t)}): {action_options_batch[i]}"""
            # t>0
            else: 

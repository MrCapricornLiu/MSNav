...
class R2RNavBatch(object):
    ...
    def make_candidate(self, current_state):
        ...
        # --- Get information from the passed state ---
        scanId = current_state.scanId
        viewpointId = current_state.location.viewpointId
        viewId = current_state.viewIndex # Current internal view index

        # --- Calculate the current relative angle baseline (for old logic) ---
        base_heading_current = (viewId % 12) * math.radians(30)
        base_elevation_current = (viewId // 12 - 1) * math.radians(30)

        adj_dict = {}
        long_id = f"{scanId}_{viewpointId}" # Cache key

        if long_id not in self.buffered_state_dict:
            # --- Internal exploration logic (unchanged) ---
            ...
            for ix in range(36):
                ...
                # Calculate the relative angle (relative to the camera orientation at exploration)
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    if (loc.viewpointId not in adj_dict or
                        ...
                        blip2_caption = None
                        # Initial image path (will be overwritten, but recorded first)
                        initial_img_path = os.path.join(self.args.img_root, scanId, viewpointId, str(ix) + '.jpg')

                        adj_dict[loc.viewpointId] = {
                            'heading': loc_heading, # Relative angle
                            'elevation': loc_elevation, # Relative angle
                            "normalized_heading": state.heading + loc.rel_heading, # Absolute angle
                            "normalized_elevation": state.elevation + loc.rel_elevation, # Absolute angle
                            'scanId': scanId,
                            'viewpointId': loc.viewpointId,
                            'pointId': ix, # Viewpoint when discovered
                            'distance': distance,
                            'idx': j + 1,
                            'position': (loc.x, loc.y, loc.z),
                            'caption': blip2_caption,
                            'image': initial_img_path, # Temporary path
                            'absolute_heading': state.heading,   # Absolute heading when discovered
                            'absolute_elevation': state.elevation, # Absolute elevation when discovered
                        }
            # --- Internal exploration end ---

            candidate = list(adj_dict.values())

            # --- New: Optimize candidate representative views ---
            if candidate:
                try:
                    # Use the current_state object to get the position information at the time of call
                    ...
                    # If an error occurs, ensure the image path is restored to the original discovery path
                    for cand in candidate:
                         cand['image'] = os.path.join(self.args.img_root, scanId, viewpointId, str(cand['pointId']) + '.jpg')
                    return candidate # Return the unoptimized list

                optimized_candidates = []
                for i_cand, cand in enumerate(candidate):
                    cand_copy = cand.copy() # Copy for operation

                    target_pos = np.array(cand['position'])
                    direction_vec = target_pos - current_pos

                    if np.linalg.norm(direction_vec) < 1e-6:
                        # The target point coincides with the current point, restore the original path and keep it
                    ...
                    # 1. Calculate the absolute world direction angle
                    ...
                    target_heading = np.arctan2(dx, dy) # Relative to +Y axis

                    # 2. Find the closest discrete view index (0-35)
                    best_ix = -1
                    ...
                    if best_ix != -1:
                        cand_copy['pointId'] = best_ix # Update pointId
                        # Build a new path using img_root
                        cand_copy['image'] = os.path.join(self.args.img_root, scanId, viewpointId, str(best_ix) + '.jpg')
                    else:
                        # If the best one is not found (theoretically should not happen), restore the original path
                         cand_copy['image'] = os.path.join(self.args.img_root, scanId, viewpointId, str(cand['pointId']) + '.jpg')

                    optimized_candidates.append(cand_copy)

                candidate = optimized_candidates # Use the optimized list
            # --- Optimization end ---

            # --- Update cache ---
            self.buffered_state_dict[long_id] = [c.copy() for c in candidate] # Store a copy list
            # --- Cache end ---

            return candidate # Return the optimized list

        else: # Read from cache
            candidate_from_buffer = self.buffered_state_dict[long_id]

            candidate_new = []
            # Recalculate the heading/elevation relative to the current viewId
            ...
            for i_cand_buf, c in enumerate(candidate_from_buffer):
                c_new = c.copy()
                # Calculate the relative angle using the absolute angles saved in the cache
                # Ensure that the buffer contains normalized_heading/elevation
                if 'normalized_heading' in c_new and 'normalized_elevation' in c_new:
                    ...
                    c_new['elevation'] = c_new['normalized_elevation'] - base_elevation_current
                     ...
                     # If the cache does not have absolute angles, the relative angle cannot be calculated, which may need to be reported or set to 0
                     c_new['heading'] = 0
                     c_new['elevation'] = 0
